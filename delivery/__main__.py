...
from src.algo.aco import AntColonyOptimization
...
parcels = pd.read_csv("delivery-data/parcels.csv", index_col=0)
distance = pd.read_csv("delivery-data/distance.csv", index_col=0)
couriers = pd.read_csv("delivery-data/couriers.csv", index_col=0)
...
for idx, courier in couriers.iterrows():
    ...
    if courier['type'] == 'pedestrian':
        parcels_in_sector = parcels_in_sector[parcels_in_sector['distance_to_hub'] <= max_delivery_radius]
    ...
    if len(assigned_parcels) > 0:
        print(f"Courier {courier['courier_id']} ({courier['type']}) assigned parcels: {list(map(int, assigned_parcels))}")
...
for idx, courier in couriers_multi_parcels.iterrows():
    ...
    print(f"Delivery for courier {courier['courier_id']} ({courier['type']}) costs: {round(cost, 2)}")
    total_cost += cost
