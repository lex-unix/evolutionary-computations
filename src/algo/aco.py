...
class AntColonyOptimization:
    def __init__(self, n_ants: int, rho: float, alpha: float, beta: float, epochs: int = 200, heuristic: NDArray = None):
        self.n_ants = n_ants
    ...
    def init_heuristic(self, heuristic: NDArray = None):
    ...
    def choose_next_node(self, location: int, unvisited: NDArray):
    ...
    def compute_tau_max(self, dist_min_cost: float):
        return (1 / self.evaporation_rate) * (1 / dist_min_cost)

    def compute_tau_min(self, tau_max: float, probability_dec: float):
    ...
    def solve(self, distance: NDArray, heuristic: NDArray = None):
        ...
        for epoch in range(self.epochs):
            for i in range(self.n_ants):
                ...
                for j in range(self.n_nodes - 1):
                    ...
                    next_node = self.choose_next_node(location, unvisited)
                    self.route[i, j + 1] = next_node

                # self.route[i, self.n_nodes] = self.start_node

            min_route, dist_min_cost = self.get_min_route()
            ...
            tau_max = self.compute_tau_max(dist_min_cost)
            tau_min = self.compute_tau_min(tau_max, probability_dec)
